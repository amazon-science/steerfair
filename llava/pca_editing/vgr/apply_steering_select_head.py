...
if __name__ == "__main__":
    ...
    parser.add_argument("--n-direction-samples", type=int, default=100)
    ...
    for subdir in os.listdir(vector_direction_dir):
        ...
        pca_directions = np.load(os.path.join(load_dir, f"pca_direction.npy"))
        pca_directions = rearrange(pca_directions, "(l h) s d -> l h s d", h=num_heads)
        pca_direction_all.append(pca_directions)

        pca_values = np.load(os.path.join(load_dir, f"pca_values.npy")).squeeze()
