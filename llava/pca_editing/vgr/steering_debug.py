...
if __name__ == "__main__":
    ...
    parser.add_argument("--combine-mode", default="qr")
    ...
    for subdir in os.listdir(vector_direction_dir):
        ...
        pca_values = np.mean(np.abs(pca_values), axis=-1)
    ...
    # print(np.dstack(pca_values_all).shape)
    per_head_component_values = get_per_head_component_value(np.dstack(pca_values_all))
    plt.imshow(per_head_component_values)
    plt.title(f'{args.question_file.split("/")[-1].split(".json")[0].split("_")[-1]}')
    plt.tight_layout()
    plt.colorbar()
    plt.savefig(f'abs 3 {args.question_file.split("/")[-1].split(".json")[0]}.png')
    plt.show()
    exit()
