...
def parse_filename(str, split):
    return str.split(f"_{split}")[0], str.split(f"noption_{n_options}_{split}_")[-1].split("_result")[0].split(".")[0]


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--n-options", type=int, required=True)
    ...
    for f in txt_files_to_parse:
        ...
        for i in range(0, len(lines), step):
            try:
                ...
                fname, params = parse_filename(f.split("/")[-1], args.split)
                # print('fname', fname)
                # print('param',params)
                # exit()
